...
load("data/colorshape_ddm_ml.RData")
...
posner_mod1_fit |>
  select(nomem_encr, acc_sim, acc) |>
  distinct() |>
  ggplot(aes(acc_sim, acc)) +
  geom_point() +
  geom_abline(aes(slope = 1, intercept = 0))

# RT model fit correlations

flanker_mod1_fit |>
  group_by(condition, percentile) |>
  mutate(r = cor(RT_sim, RT)) |>
  select(condition, percentile, r) |>
  distinct()
...
globallocal_mod1_fit |>
  group_by(condition, percentile) |>
  mutate(r = cor(RT_sim, RT, use = 'pairwise.complete.obs')) |>
  select(condition, percentile, r) |>
  distinct()
...
# Acc model fit correlations

flanker_mod1_fit |>
  filter(percentile == "RT_25") |>
  group_by(condition) |>
  mutate(r = cor(acc_sim, acc)) |>
  select(condition, percentile, r) |>
  distinct()
...
globallocal_mod1_fit |>
  filter(percentile == "RT_25") |>
  group_by(condition) |>
  mutate(r = cor(acc_sim, acc, use = 'pairwise.complete.obs')) |>
  select(condition, percentile, r) |>
  distinct()
