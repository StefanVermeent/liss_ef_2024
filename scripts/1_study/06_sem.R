...
mod_v1 <-
  ' # Latent factors
    ...
    as =~ 1*cs_sw_v_c + gl_sw_v_c + as_sw_v_c
...
fit_v1_sum <- summary(fit_v1, standardized = TRUE, fit.measures = T)
fit_v1_fitstats <- fit_v1_sum$fit[c('chisq', 'df', 'pvalue', 'cfi.robust', 'rmsea.robust', 'rmsea.ci.lower.robust', 'rmsea.ci.upper.robust')]
...
mod_v2 <-
  ' # Latent factors
    ps =~ 1*pos_v_c + flanker_inc_v_c + flanker_con_v_c + simon_inc_v_c + simon_con_v_c + cs_sw_v_c + cs_rep_v_c + gl_sw_v_c + gl_rep_v_c + as_sw_v_c + as_rep_v_c
    ef =~ 1*flanker_inc_v_c + 1*simon_inc_v_c + cs_sw_v_c + gl_sw_v_c + as_sw_v_c
...
fit_v_compare <- anova(fit_v1, fit_v2) |> as_tibble() |> mutate(model = c("model1", "model2"))
...
fit_a1_sum <- summary(fit_a1, standardized = TRUE, fit.measures = T)
fit_a1_fitstats <- fit_a1_sum$fit[c('chisq', 'df', 'pvalue', 'cfi.robust', 'rmsea.robust', 'rmsea.ci.lower.robust', 'rmsea.ci.upper.robust')]
...
fit_a2_sum <- summary(fit_a2, standardized = TRUE, fit.measures = T)
fit_a2_fitstats <- fit_a2_sum$fit[c('chisq', 'df', 'pvalue', 'cfi.robust', 'rmsea.robust', 'rmsea.ci.lower.robust', 'rmsea.ci.upper.robust')]

fit_a_compare <- anova(fit_a1, fit_a2) |> as_tibble() |> mutate(model = c("model1", "model2"))
...
fit_t1_sum <- summary(fit_t1, standardized = TRUE, fit.measures = T)
fit_t1_fitstats <- fit_t1_sum$fit[c('chisq', 'df', 'pvalue', 'cfi.robust', 'rmsea.robust', 'rmsea.ci.lower.robust', 'rmsea.ci.upper.robust')]
...
mod_dep1 <-
  '
    ...
    common_v ~ p_scar_m + age + sex + edu + child_adv
    common_a ~ p_scar_m + age + sex + edu + child_adv
'
...
## 5.1 Primary model (with deprivation as confounder)
...
mod_thr1 <-
  '
    ...
    age           ~~ p_scar_m + vict_sum + neigh_thr02_m + sex + child_adv
    p_scar_m      ~~ vict_sum + neigh_thr02_m + sex + child_adv
    vict_sum      ~~ neigh_thr02_m + sex + child_adv
    neigh_thr02_m ~~ sex + child_adv
    sex           ~~ child_adv
    ...
    gen_v ~ vict_sum + neigh_thr02_m + p_scar_m + age + sex + edu + child_adv
    gen_a ~ vict_sum + neigh_thr02_m + p_scar_m + age + sex + edu + child_adv
    gen_t ~ vict_sum + neigh_thr02_m + p_scar_m + age + sex + edu + child_adv

    common_v ~  vict_sum + neigh_thr02_m + p_scar_m + age + sex + edu + child_adv
    common_a ~  vict_sum + neigh_thr02_m + p_scar_m + age + sex + edu + child_adv
'
