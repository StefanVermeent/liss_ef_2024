...
fig4 <-
  ...
  geom_errorbar(aes(ymin = ci.lower, ymax = ci.upper, color = ddm_parameter, linetype = .data[['linetype']]), width = 0.2, size = 1) +
...
expl_ts_cor_inh_fig <- expl_ts_cor_inh |>
  ...
  geom_text(aes(lhs, rhs, label = round(est.std, 2)), color = "white", size = 3) +
  coord_fixed() +
  theme_void() +
  guides(fill = 'none') +
...
table3 <- bind_rows(
  fit_v2_fitstats |> as_tibble_row() |> mutate(model = "Drift rate model 1"),
  fit_a3_fitstats |> as_tibble_row() |> mutate(model = "Boundary separation model 1"),
...
save(table1, table2, table3, file = "analysis_objects/rendered_tables.RData")
save(fig4, fig5, fig6, fig7, file = "analysis_objects/rendered_figures.RData")
